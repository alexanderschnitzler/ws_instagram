...
class Instagram {
    ...
    public function __construct($config) {
        if (is_array($config)) {
            ...
            throw new \MetzWeb\Instagram\InstagramException('Error: __construct() - Configuration data is missing.');
        }
    }
    ...
    public function getLoginUrl($scopes = array('basic')) {
        if (is_array($scopes) && count(array_intersect($scopes, $this->_scopes)) === count($scopes)) {
            return self::API_OAUTH_URL . '?client_id=' . $this->getApiKey() . '&redirect_uri=' . $this->getApiCallback() . '&scope=' . implode('+', $scopes) . '&response_type=code';
        }

        throw new \MetzWeb\Instagram\InstagramException("Error: getLoginUrl() - The parameter isn't an array or invalid scope permissions used.");
    }
    ...
    public function getUserFollows($id = 'self', $limit = 0) {
        ...
        return $this->_makeCall('users/self/follows', TRUE, $params);
    }
    ...
    public function getUserFollower($id = 'self', $limit = 0) {
        ...
        return $this->_makeCall('users/self/followed-by', TRUE, $params);
    }
    ...
    public function modifyRelationship($action, $user) {
        ...
        throw new \MetzWeb\Instagram\InstagramException('Error: modifyRelationship() | This method requires an action command and the target user id.');
    }
    ...
    public function pagination($obj, $limit = 0) {
        ...
        throw new \MetzWeb\Instagram\InstagramException("Error: pagination() | This method doesn't support pagination.");
    }
    ...
    protected function _makeCall($function, $auth = FALSE, $params = NULL, $method = 'GET') {
        if (!$auth) {
            ...
            if (!isset($this->_accesstoken)) {
                throw new \MetzWeb\Instagram\InstagramException("Error: _makeCall() | $function - This method requires an authenticated users access token.");
            }
        ...
        if (!$jsonData) {
            throw new \MetzWeb\Instagram\InstagramException('Error: _makeCall() - cURL error: ' . curl_error($ch));
        }
    ...
    private function _makeOAuthCall($apiData) {
        ...
        if (!$jsonData) {
            throw new \MetzWeb\Instagram\InstagramException('Error: _makeOAuthCall() - cURL error: ' . curl_error($ch));
        }
